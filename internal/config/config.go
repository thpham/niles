...
import (
	...
	"github.com/thpham/niles/internal/defaults"
)

type ConfigWorkflowHistContainer struct {
	Starttime string
	Endtime string
	Timeout uint
}
...
type ConfigContainer struct {
	...
	Tick         uint
	WorkflowHist ConfigWorkflowHistContainer
	TemplateDirs []string
}
...
func (cc *ConfigContainer) GetConfig() error {
	...
	// NILES_CONF content, if exists
	if nilesConf, exists := os.LookupEnv(defaults.EnvConfVarName); exists && nilesConf != "" {
		// NILES_CONF set
		cfgPaths = append(cfgPaths, nilesConf)
	}

	// $XDG_CONFIG_HOME/niles/niles.conf
	...
	if len(cc.WorkflowHist.Starttime) == 0 {
		cc.WorkflowHist.Starttime = defaults.HistStart
	}
	// if unset (==0), set to default
	if cc.WorkflowHist.Timeout < 1 {
		cc.WorkflowHist.Timeout = defaults.HistTimeout
	}
	cc.testNsetBinPaths()
